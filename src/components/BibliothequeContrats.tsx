...
import { ContractSubCategory } from '../types';
...
interface BibliothequeContratsProps {
  onNotificationClick: () => void;
  notificationCount: number;
}

export default function BibliothequeContrats({ onNotificationClick, notificationCount }: BibliothequeContratsProps) {
  const [selectedCategory, setSelectedCategory] = useState<ContractSubCategory>('PER');
  ...
  const {
    ...
    loadDocuments,
    isMockDocument,
  } = useLibraryDocuments({ category: 'Contrats', subCategory: selectedCategory });
  ...
  const perCount = mockLibraryDocuments.filter(d => d.sub_category === 'PER').length;
  const avCount = mockLibraryDocuments.filter(d => d.sub_category === 'Assurance Vie').length;
  const prevoyanceCount = mockLibraryDocuments.filter(d => d.sub_category === 'Prévoyance').length;

  const getCategoryCount = (cat: ContractSubCategory) => {
    switch (cat) {
      case 'PER': return perCount;
      case 'Assurance Vie': return avCount;
      case 'Prévoyance': return prevoyanceCount;
      default: return 0;
    }
  };
    ...
    <div className="flex-1 overflow-auto">
      <header className="glass-card ml-20 mr-4 lg:mx-8 mt-4 md:mt-6 lg:mt-8 px-4 md:px-6 lg:px-8 py-4 md:py-5 flex items-center justify-between floating-shadow">
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-500 font-light mt-1 hidden sm:block">
            Documents contractuels et produits
          </p>
        </div>
        <button
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <select
                ...
                onChange={(e) => setSelectedCategory(e.target.value as ContractSubCategory)}
                className="px-4 py-2.5 bg-white border border-gray-200 rounded-2xl text-sm font-light focus:outline-none focus:ring-2 focus:ring-blue-400/50 min-w-[160px]"
              >
                <option value="PER">PER ({getCategoryCount('PER')})</option>
                <option value="Assurance Vie">Assurance Vie ({getCategoryCount('Assurance Vie')})</option>
                <option value="Prévoyance">Prévoyance ({getCategoryCount('Prévoyance')})</option>
              </select>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-sm text-gray-500 font-light mb-6">
                {searchTerm ? 'Aucun résultat pour votre recherche' : `Aucun document ${selectedCategory} disponible`}
              </p>
      ...
      {showUploadModal && currentUserId && (
        <UploadDocumentModal
          ...
          organizationId="1"
          uploadCategory="Contrats"
          uploadSubCategory={selectedCategory}
